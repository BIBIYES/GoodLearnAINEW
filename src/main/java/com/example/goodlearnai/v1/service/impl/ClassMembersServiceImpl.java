...
public class ClassMembersServiceImpl extends ServiceImpl<ClassMembersMapper, ClassMembers>
    ...
    public Result<String> intoClass(ClassJoinRequest request) {
        ...
        if (request.getClassId() == null) {
            log.warn("未提供班级ID: userId={}", userId);
            return Result.error("请选择要加入的班级");
        }

        Class classEntity = classMapper.selectById(request.getClassId());
        if (classEntity == null) {
            log.warn("班级不存在: classId={}", request.getClassId());
